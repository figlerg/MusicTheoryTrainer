...
class Sound(private val player: TonePlayer, val enabled: Boolean, private val voice: Voice = Voice.PLUCK) {

    val playing: StateFlow<Boolean> get() = player.playing

    fun play(midi: Int) {
        if (enabled) player.play(midi, voice)
    }
    ...
    fun playSequence(midis: List<Int>, secondsEach: Double = 0.4) {
        if (!enabled) return
        player.playNotes(midis.mapIndexed { i, midi -> PlayNote(midi, i * secondsEach, if (i == midis.lastIndex) 1.0 else secondsEach) }, voice)
    }
